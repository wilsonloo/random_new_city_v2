...
random = function(region, cell_w, cell_h)
    ...
    elseif region.node then
        ...
        -- add first node
        local cell_x, cell_y = rand_cell(0, 0, region.w, region.h, cell_w, cell_h)
        assert(cell_x)
        assert(cell_y)
        local node = {
            x = cell_x,
